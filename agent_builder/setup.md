...
## Tool 5 — `get_league_context`

**This is the aggregation tool** (docs/sow.md hard requirement 4a). Everything
else queries or joins documents; this one computes across the whole index with
ES|QL `STATS`.

It exists so the agent can scale a number instead of just reporting it — "a
12.2 net rating, best in the league, against a league average of 0.0" lands in
a demo where "a 12.2 net rating" does not.

```json
PUT kbn:/api/agent_builder/tools/get_league_context
{
  "type": "esql",
  "description": "Get league-wide aggregate context: number of teams, average/best/worst net rating, and average scoring. Use this to describe where a specific team sits relative to the rest of the league before quoting its raw numbers.",
  "configuration": {
    "query": "FROM nba_team_stats | STATS teams = COUNT(*), avg_net_rating = ROUND(AVG(net_rating), 2), best_net_rating = ROUND(MAX(net_rating), 2), worst_net_rating = ROUND(MIN(net_rating), 2), avg_points = ROUND(AVG(points_per_game), 1), avg_points_allowed = ROUND(AVG(points_allowed_per_game), 1)"
  }
}
```

> Returns exactly one row. `avg_net_rating` should be ~0.0 — net rating is a
> point differential, counted once for the winner and once against the loser,
> so the league sums to zero. If it isn't, ingest has double-counted or
> dropped games, and every downstream probability is suspect. That makes this
> tool a useful first call when the numbers look wrong.

More aggregations, including grouped ones and a per-game bookmaker-spread
query, are in [../esql/league_aggregates.esql](../esql/league_aggregates.esql).
